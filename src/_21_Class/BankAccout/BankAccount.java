...
public class BankAccount {
    ...
    BankAccount () {
        System.out.println("계좌가 새설되었습니다. 현재 잔액: " + balance + "원");
    }

    public void deposit(int amount) {
        if (amount > 0) {
            balance += amount;
            System.out.println("계좌에" + amount + "원 입금되었습니다. 현재 잔액: " + balance);
        }else  {
    ...
    public void withdraw(int amount) {
        if (amount > 0 && amount <= balance) {
            ...
            System.out.println("출금이 불가능 합니다. 남은 잔액은" + balance + "입니다");
        }
    ...
    // 두번째는 setter는 검증 단계를 포함하지 않는다.
    // 하지만 메소드는 보통은 검증단계를 포함 할 수 있다.
